...
TOUPCAM_OPTION_TRIGGER               = 0x0b   # 0 = video mode, 1 = software or simulated trigger mode, 2 = external trigger mode, default value = 0
...
TOUPCAM_OPTION_BINNING               = 0x17   # binning, 0x01 (no binning), 0x02 (add, 2*2), 0x03 (add, 3*3), 0x04 (add, 4*4), 0x82 (average, 2*2), 0x83 (average, 3*3), 0x84 (average, 4*4)
...
TOUPCAM_OPTION_AUTOEXP_THRESHOLD     = 0x29   # threshold of auto exposure, default value: 5, range = [5, 15]
...
# get the version of this dll, which is: 42.$WCREV$.$WCDATE=%Y.%m%d$
    @classmethod
    def Version(cls):
        cls.__initlib()
...
# FourCC, bitsperpixel
    ...
    def put_RealTime(self, bEnable):
        self.__lib.Toupcam_put_RealTime(self.__h, ctypes.c_int(1 if bEnable else 0))
